...
1. **우선순위 (1-5)**: 발신자 중요도, 내용 긴급도, 마감일, **수신유형** 고려
...
**수신유형에 따른 우선순위 조정**:
- 📩 직접수신 (direct): 기본 우선순위 유지 (내가 주 담당자)
- 📋 참조(CC): 우선순위 -1 적용 (직접 수행 필요 없음)
- 👥 그룹메일: 우선순위 -1 적용 (전체 공지/안내)
- **단, 내용이 정말 중요하면 CC/그룹이어도 P3-4 가능**
...
### 우선순위 (P1-P5) - 일관성 있게 적용

| 점수 | 레벨 | 기준 | 예시 |
|------|------|------|------|
| P5 | 최우선 | 팀장/직속상관 직접 요청, 마감 24시간 내 | 장수진 팀장 요청, 긴급 회의 |
| P4 | 긴급 | 참석 필수 회의, 마감 1주 내, 업무 요청 | 교육 안내, 월례회의 |
| P3 | 보통 | 일반 업무 공유, 참조용, FYI | 동료 간 정보 공유 |
| P2 | 낮음 | 전사 공지, 시스템 알림 | 다이어리 배포, 보안 알림 |
| P1 | 최저 | 뉴스레터, 마케팅, 자동메일, 회수 메일 | 학회 뉴스레터, 광고 |

**판단 규칙 (순서대로 적용)**:
1. 발신자가 팀장(Soojin Jang)이면 → P5
2. 마케팅/뉴스레터/자동발송/회수 메일 → P1
3. 참석 필수 회의/교육 → P4
4. 전사 공지/시스템 알림 → P2
5. 그 외 → P3
6. **수신유형 조정**: CC/그룹메일은 최종 우선순위에서 -1 (단, 중요 내용이면 유지)

### 수신유형별 우선순위 조정

| 수신유형 | 의미 | 우선순위 조정 |
|----------|------|---------------|
| 📩 직접수신 | 내가 To 필드에 지정됨 | 유지 (내가 주 담당자) |
| 📋 참조(CC) | CC로 참조 수신 | -1 (직접 수행 불필요) |
| 👥 그룹메일 | 메일링리스트/전체 발송 | -1 (전체 공지) |

**중요**: 수신유형은 기본 조정값이며, 내용이 중요하면 AI 판단으로 우선순위 유지/상향 가능
