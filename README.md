...
### v0.5.0 (Latest)
- ✅ **3차원 우선순위 스코어링** - Claude Code skill 기반 **(NEW!)**
  - Sender Importance (0-100): 관계 깊이 + 직급/역할 + 최근 활동
  - Content Urgency (0-100): 시간 민감도 + 액션 필요도 + 내용 중요도
  - Context Modifiers (-20~+20): 첫 연락, 긴 스레드, CC, 자동발송 등
- ✅ **발신자 중요도 관리** - 발신자 관리 Sheets 탭 **(NEW!)**
  - 자동 스코어링: P4-5 비율 + 교신 빈도 + 발신 가중치 + 최근 활동
  - 수동 등급 설정: VIP/중요/보통/낮음/차단
- ✅ **Gmail 라벨 자동 관리** - 8개 라벨 자동 생성 및 적용 **(NEW!)**
  - 상태: 답장필요, 답장불필요, 답장완료
  - 우선순위: P1-최저, P2-낮음, P3-보통, P4-긴급, P5-최우선

### v0.4.0
- ✅ **API 비용 $0** - Claude Code 대화로 처리
- ✅ **Gmail HTML 초안** - 서식 유지 (볼드, 색상, 서명)
- ✅ **하이브리드 아키텍처** - Gmail 초안 + Sheets 워크플로우
  - Gmail = 콘텐츠 저장소 (서식 보존)
...
- **Gmail** = 콘텐츠 저장소 (서식 보존)
- **Sheets** = 워크플로우 대시보드 (관리)
- **발송 시 Gmail 초안을 그대로 전송** (사용자 수정사항 반영)

---

### 상세 단계

프로그램을 실행하면 **자동으로 4~6단계**로 진행됩니다:
...
### 3. Google Sheets 통합 (v0.4.0 업데이트)
...
- **Gmail 초안 (클릭하여 열기)** ← NEW: 하이퍼링크
- 발송여부 (체크박스)
- Draft ID, Thread ID (숨김)

**워크플로우**:
1. 프로그램 실행 → 이메일 자동 분류
2. Gmail HTML 초안 생성 (서식 포함)
3. Sheets에 초안 링크 기록
